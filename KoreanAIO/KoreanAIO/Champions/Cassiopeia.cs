...
namespace KoreanAIO.Champions
{
    public class Cassiopeia : ChampionBase
    {
        ...
        public Cassiopeia()
        {
            ...
            {
                if ((ModeManager.LastHit || ModeManager.Harass) && sender.IsEnemy && args.End.Distance(MyHero, true) <= 800 && args.End.Distance(MyHero, true) > 500)
                    {
                        W.Cast(sender);
                    }
        ...
        protected override void KillSteal(Menu menu)
        {
            foreach (var enemy in UnitManager.ValidEnemyHeroesInRange.Where(h => h.HealthPercent <= 40f))
            {
                ...
                if (menu.CheckBox("E") && (result.E || E.IsKillable(enemy)) && ModeManager.Combo)
                {
                    E.Cast(enemy);
                }
